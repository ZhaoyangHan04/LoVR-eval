...
def load_model():
    ...
    # 使用配置名初始化模型
    model = pe.CLIP.from_config('PE-Core-B16-224', pretrained=False)  # 不下载

    # 加载本地权重
    weights_path = "/mnt/public/***/***/LOVR/models/PE-Core-B16-224/PE-Core-B16-224.pt"
...
def encode_text(args):
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(all_text), BATCH_SIZE):
            batch_text = processor(all_text[i:i+BATCH_SIZE]).cuda()
            # fake_imgs = torch.zeros(len(batch_text), 3, image_size, image_size).cuda()
            fake_imgs = torch.zeros(1, 3, 224, 224).cuda()
            with torch.no_grad(), torch.autocast("cuda"):
                _, feat, _ = model(fake_imgs, batch_text)
        ...
        with torch.no_grad():
            with _timer("encode_image_cost"):
                ...
                for video_inputs in video_inputs_list:
                    ...
                    fake_text = torch.zeros(1, 32, dtype=torch.long).cuda()

                    for i in range(0, num_frames, BATCH_SIZE):
                        batch_imgs = video_inputs[i:i+BATCH_SIZE]  # [B, 3, 224, 224]

                        with torch.no_grad(), torch.autocast("cuda"):
                            image_features, _, _ = model(batch_imgs, fake_text)  
