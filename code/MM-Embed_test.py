...
WEIGHT_PATH = '/mnt/public/***/***/LOVR/models/MM-Embed' #
...
def load_model():
    print("begin loading MM-Embed model")
    model = AutoModel.from_pretrained(
        WEIGHT_PATH,  # 你自己的权重目录
...
def encode_text(args):
    ...
    # >>>>>>>>>>>>>>>>>>>> encode api <<<<<<<<<<<<<<<<<<<<
    INSTRUCTION = "Retrieve the most relevant image given the textual description."
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(all_text), BATCH_SIZE):
            ...
            batch_inputs = [{'txt': t} for t in batch_texts]   # dict 格式
    ...
    for video in pbar:
        ...
        with _timer("preprocess_cost"):
            ...
            for frame_list in images:
                ...
                video_inputs_list.append(pil_imgs)
        ...
        with torch.no_grad():
            with _timer("encode_image_cost"):
                ...
                text_dict = torch.load(TEXT_RESULT_PATH, map_location=device)
                text_feat_dict = {src: feat for src, feat in zip(text_dict['source'], text_dict['feat'])}
                
                for pil_imgs, full, clip in zip(video_inputs_list, full_src, clip_src):
                    ...
                    for i in range(0, len(pil_imgs), BATCH_SIZE):
                        ...
                        # ✅ 改成传 dict，字段名是 "img"
                        batch_dicts = [{"img": img} for img in batch_imgs]
                        ...
                        batch_feats = outputs["hidden_states"]
                        frame_feats.append(batch_feats)
                    ...
                    if frame_feats is not None:  
                        # 直接查找对应的文本特征
                        if current_src in text_feat_dict:
                            text_feat = text_feat_dict[current_src]  # 对应的单个文本特征
                            text_feat = text_feat.unsqueeze(0)  # [1, hidden_dim]
                            
                            # 计算帧特征与文本特征的相似度 [num_frames, 1]
                            print("frame_feats device:", frame_feats.device, frame_feats.dtype)
                            print("text_feat device:", text_feat.device, text_feat.dtype)
                            print('Device now: ', device)
                            
                            frame_feats = frame_feats.to(device)
                            text_feat = text_feat.to(device)
                            
                            similarity = frame_feats @ text_feat.T  # 矩阵乘法
                            similarity = similarity.squeeze(1)  # [num_frames]
                            
                            # 选择相似度最高的帧
                            max_sim, best_frame_idx = similarity.max(dim=0)
                            video_feat = frame_feats[best_frame_idx].unsqueeze(0)  # [1, hidden_dim]
