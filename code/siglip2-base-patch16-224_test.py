...
RUN_NAME = 'siglip2-base-patch16-224' #
...
WEIGHT_PATH = f'/mnt/public/***/***/LOVR/models/{RUN_NAME}' #
...
from transformers import AutoModel, AutoProcessor, CLIPModel, BitsAndBytesConfig
...
def load_model():
    ...
    processor = AutoProcessor.from_pretrained(WEIGHT_PATH, use_fast = True)
    model.eval().cuda()
    ...
    with torch.no_grad():
        for i in range(0, len(all_text), BATCH_SIZE):
            ...
            inputs = processor(
                ...
                padding="max_length", 
                truncation=True,      
                max_length=64).to('cuda')     # 明确最大长度).to("cuda")
    ...
    for video in pbar:
        ...
        with _timer("preprocess_cost"):
            ...
            for frame_list in images:
                ...
                video_inputs = processor(images = pil_imgs, return_tensors='pt', padding = 'max_length', max_length=64)
                video_inputs = {k: v.cuda().to(torch.bfloat16) for k, v in video_inputs.items()}
        ...
        with torch.no_grad():
            with _timer("encode_image_cost"):
                ...
                for video_inputs in video_inputs_list:
                    ...
                    num_frames = video_inputs['pixel_values'].shape[0]  # 第0维是帧数

                    for i in range(0, num_frames, BATCH_SIZE):
                        batch_dict = {k: v[i:i+BATCH_SIZE] for k, v in video_inputs.items()}
                        outputs = model.get_image_features(**batch_dict)
                        frame_feats.append(outputs)
