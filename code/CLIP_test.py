...
def load_model():
    print('begin loading CLIP model')
    model, preprocess = clip.load(WEIGHT_PATH, device=device, jit=False)
    model.eval()
...
def encode_text(args):
    ...
    # >>>>>>>>>>>>>>>>>>>> encode api <<<<<<<<<<<<<<<<<<<<
    text = clip.tokenize(all_text, truncate=True).to(device)

    BATCH_SIZE = 512
    with torch.no_grad():
        text_features = [model.encode_text(text[i : i+BATCH_SIZE]) for i in range(0, len(text), BATCH_SIZE)]
...
def encode_video(args):
    ...
    for video in pbar:
        ...
        with _timer('preprocess_cost'):
            # step1: 逐视频 → 逐帧 → preprocess
            video_inputs_list = []  # 存放每个视频的预处理后的帧
            for frame_list in images:   # 每个元素是一个视频（帧数组）
                frame_inputs = []
                for fr in frame_list:
                    pil_img = Image.fromarray(fr[:, :, ::-1])  # BGR -> RGB
                    frame_inputs.append(preprocess(pil_img))
                frame_inputs = torch.stack(frame_inputs).to(device)  # [num_frames, 3, 224, 224]
                video_inputs_list.append(frame_inputs)

        with torch.no_grad():
            with _timer("encode_image_cost"):
                video_features = []
                for frame_inputs in video_inputs_list:
                    frame_feats = model.encode_image(frame_inputs)
